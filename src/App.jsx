        ...
        fetch('https://yalantis-react-school-api.yalantis.com/api/task0/users')
            ...
            .then(data => {
                ...
                months = months.map(number => {
                    ...
                    else if (number >= 7) return 'green';
                    else if (number >= 3) return 'blue';
                    else return 'gray';
                })
        ...
        return (
            <React.Fragment>
                <div className='months'>
                    {months.map((color, index) => <Month color={color} month={index} key={index} handleOnMouseEnter={this.handleMonthOnMouseEnter} handleOnMouseLeave={this.handleMonthOnMouseLeave} />)}
                </div>
                <Users users={currentMonthData.length ? currentMonthData : users} />
            </React.Fragment>
        )
    }
}
